...
class DrivingBenchmark(object):
                    ...
                    (result, reward_vec, control_vec, final_time, remaining_distance, col_ped,
                     col_veh, col_oth, number_of_red_lights, number_of_green_lights) = \
                        self._run_navigation_episode(
                            ...
                            perturb_speed=experiment_suite.perturb_input_speed)
    ...
    def _run_navigation_episode(
        ...
        processed_image_queue = Queue(maxsize=g_conf.NUMBER_FRAMES_FUSION)
        ...
        is_time_out = False
        not_count = 0
        agent.client_side_process.reset()
        ...
        while not fail and not success:
            ...
            if not processed_image_queue.empty():
                processed_image_queue.get()
            ...
            if g_conf.NUMBER_PREVIOUS_ACTIONS > 0:
                control, original_image, processed_image = \
                    agent.run_step(measurements, sensor_data,
                                   list(original_image_queue.queue), list(processed_image_queue.queue),
                                   directions, target,
                                   previous_actions_list=list(previous_actions_queue.queue),
                                   avoid_stop=avoid_stop, perturb_speed=perturb_speed)
            else:
                control, original_image, processed_image = \
                    agent.run_step(measurements, sensor_data,
                                   list(original_image_queue.queue), list(processed_image_queue.queue),
            ...
            while not processed_image_queue.full():
                processed_image_queue.put(processed_image)
            ...
            # save processed video if the flag is activate
            self._recording.write_processed_video(processed_image)

            # save measurements and controls if the flag 'policy_roll_out' is activated
            self._recording.policy_roll_out(measurements, control, episode_name, frame, directions)
