...
class Recording(object):
    ...
    def __init__(self,
        ...
        if not os.path.exists(os.path.join(results_dir, '_benchmarks_results')):
            os.mkdir(os.path.join(results_dir, '_benchmarks_results'))

        # Generate the full path for the log files
        self._path = os.path.join(results_dir,
                                  '_benchmarks_results'
                                  , name_to_save
                                  )
    ...
    def policy_roll_out(self, measurements, control, episode_name, frame, directions):
        """
        ...
        if self._policy_roll_out:
            ...
            with open(control_path, 'w') as fo:
                json.dump(control_dict, fo, sort_keys=True, indent=4)
